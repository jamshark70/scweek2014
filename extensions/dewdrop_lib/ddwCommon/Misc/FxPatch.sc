	...
	*initClass {
	...
	playToMixer { arg m, atTime = nil, callback;
		...
		{	args[busArgIndex].respondsTo(\value_).if({
				...
				argsForSynth.put(busArgIndex, m.inbus.index);
			});
		}.try({
		...
		// argsForSynth.debug("argsForSynth");

		// cxx does not guarantee unique defnames for unique arg sets
		this.asSynthDef;
		synthDef.name = synthDef.name ++ UniqueID.next;

		this.play(m.effectgroup, atTime,
			SharedBus(def.rate, m.inbus.index, numChannels, m.server), callback);
		// keep some debug code for a bit
		// bundle = AbstractPlayer.bundleClass.new;
		// this.prepareToBundle(m.effectgroup, bundle, false, SharedBus(def.rate, m.inbus.index, numChannels, m.server)).spawnToBundle(bundle);
		// bundle.messages.do { |row| row.postln };
		// m.server.listSendBundle(nil, bundle);
	}
}
