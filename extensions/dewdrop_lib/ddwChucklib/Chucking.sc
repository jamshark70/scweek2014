...
MCG : AbstractChuckArray {
	*persistent { ^true }
	...
	bindSY { |sy, adverb(\chan)|
		var	mix;
		sy.env[adverb].isMixerChannel.if({
			value.mixer_(sy.env[adverb]);
		}, {
				// DON'T COMMIT THIS: IT'LL NEVER WORK
			mix = sy.v.tryPerform(\asMixer);
			mix.isMixerChannel.if({ value.mixer = mix },
				{ "SY's target is not a MixerChannel. Can't bind into MCG.".postln; });
		});
	}
}
