...
VoicerMIDIController : AbstractMIDIControl {
	...
	destination_ { arg dest;
		// pure voodoo: without the (no-op) "catch" func, displayNameSet does not take effect here
		{	destination.midiControl = nil;	// break old connection
			// "cleared old midiControl".debug;
			destination.displayNameSet;
			// "called displayNameSet".debug;
		}.try; // { |error| nil /*error.reportError; "^^ try to unmap old destination".debug*/ };
	...
	draggedIntoVoicerGCGUI { |newDest|
		...
		(newDest.model.midiControl !== this).if({
			...
			{ newDest.model.midiControl.destination_(defaultDest); }.try { nil };
