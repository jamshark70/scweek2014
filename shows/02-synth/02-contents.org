...
* Modal synthesis: Ringing filters
** Ringing filters
*** What's a ringing filter?
...
~filter = { |ffreq = 2000, rq = 0.1|
   BPF.ar(~oscil.ar, ffreq, rq, 10.dbamp).dup
};
~filter.play;
...
~filter = { |ffreq = 2000, ringtime = 0.5|
   Ringz.ar(~oscil.ar, ffreq, ringtime, 0.01).dup
};
#+end_src
...
**** Simplest explanation of modal synthesis:
