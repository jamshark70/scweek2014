...
#+latex: \setcounter{lstlisting}{38}
...
**** In Listing [[pbind1]], change \clss{Pbind} to \clss{DebugPbind}.
     ...
     2. \ci{\textbackslash freq} gets a random value, e.g. 133.25.
...
In graphical patching environments, you would have to create each of
the \cd{[*\textasciitilde]} amplifiers by hand (unless you want to
automate object creation using JavaScript). Adding or removing a
channel can force you to make cascading changes through large sections
of the patch.
...
Multichannel expansion is useful even for ordinary synthesis: Because
it's more convenient in SC, you'll do more of it and get more
interesting sounds for less work.
...
returns =/usr/local/share/SuperCollider=; in Windows XP, it's
=C:\Program Files\SuperCollider-3.6.6=.
