...
\ugen{BufChannels}. But you /may not/ use this in the \ci{numChannels}
input. If you do, you'll get this error: "ERROR: Primitive
'\textunderscore BasicNew' failed. Index not an Integer."
\cd{b.numChannels} is valid because it uses information already
...
**** Parameters needed for single \glspl{grain}:
     ...
     - *Duration*.
     - *Position* within the buffer, as a \egls{normalize}d range.
...
*** GrainBuf UGen
**** All-purpose granulator: \ugen{GrainBuf}.
...
*** Plan							   :noexport:
    - Parameters, one by one:
      - Trigger freq vs. graindur
      - Advance thru buffer (Phasor)
      - Speed of advance
      - Grain rate
