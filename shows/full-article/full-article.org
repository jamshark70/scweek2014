...
**** Overview (BTW change standard to canonical)
...
"standard" SuperCollider style is different in some important ways.

Why not simply teach the standard style? When approaching any
programming language, small code blocks are easier to understand,
while larger ones, even ten lines, may be scary at first. Standard
...
Code examples appear in a monospaced sans-serif font (Inconsolata):
\cd{Server.local.boot}.
