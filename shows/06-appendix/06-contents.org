...
** Model-View-Controller: What and why
...
*** TODO Notes on listeners			   :B_ignoreheading:noexport:
    :PROPERTIES:
    :BEAMER_env: ignoreheading
    :END:
...
**** \clss{SimpleController} can register a function to update the slider.
     - How to set the slider's value?\pause\\
       \cd{z.value = newValue;}
     - Where does the new value come from?\pause\\
       \cd{setArray[1]}\pause
#+name: mvc4
...
*** Maybe not							   :noexport:
**** Information flow:
     - The controllers "own" the views.
       - The controller talks to /its own/ views.
       - The views talk only to their own controllers.
     - A controller communicates changes directly to its model.
     - The model broadcasts its changes.\\
       It never talks directly to a specific controller.
...
*** TODO Wrap up						   :noexport:
    - Chart of data flow
    - Adding the OSC controller was easy because we set up a good structure first
