...
** Code panel
*** Run code: =Ctrl-Return=
    - Runs a code block, selected code or the current line
    - /Code block/: =(= at the beginning, =)= at the end
...
*** Auto-completion
    ...
    1. Type =Sin=. SC knows which /classes/ begin with those letters. Choose =SinOsc=.
    2. Type a dot. SC knows which /messages/ =SinOsc= understands. Choose =ar=.
    3. Type =(=. SC knows the inputs of =SinOsc.ar= and shows them to you.
*** Keystrokes for auto completion:
    - Ctrl-space :: Complete a name.
    - Ctrl-shift-space :: Complete a parameter list inside =()=.

** Let's make some noise!
   1. Boot the audio server: =Ctrl-B= \\
