...
**** In this simple case, we want one channel to feed into the filter.
     So, \ugen{Mix} the three signals into one: \\
     \cd{Mix(Saw.ar([1, 2, 3]))} \to \cd{a Sum3}
...
* Effects common in subtractive synthesis
** Chorus
** Distortion
* Glossary							   :noexport:
# #+LaTeX: \glsaddall
