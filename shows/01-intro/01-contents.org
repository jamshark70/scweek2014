...
* Glossary entries				   :B_ignoreheading:noexport:
  :PROPERTIES:
  :BEAMER_env: ignoreheading
  :END:
Where are my damn results?
#+call: ugengloss01
#+results: ugengloss01
...
**** A UGen's \te{output range} is one of its most important characteristics.
     ...
     - \ci{SinOsc}'s range is \te{bipolar}: $-1.0$--$1.0$.
...
*** Writing ranges in SC
    - Long form: \cd{ControlSpec(low, high, curve, step, default)}
...
**** The most basic types of filters are:
     - Lowpass \ci{LPF} :: Allow low frequencies to pass through. Cut high frequencies.
     - Highpass \ci{HPF} :: Allow high frequencies to pass through. Cut low frequencies.
     - Bandpass \ci{BPF} :: Allow a range in the middle to pass through. Cut high and low.

*** Butterworth filters
**** \ci{LPF} and \ci{HPF} are \te{Butterworth filters}.
...
*** BPF: Bandwidth
**** \ci{BPF} needs to know how wide a band.
...
#+End_src
**** Also try \ci{RHPF} (on your own).

*** Subtractive synthesis: Oscillator summary
**** Oscillators so far
     - \ci{Saw} :: Band-limited sawtooth
     - \ci{VarSaw} :: /Non/-band-limited sawtooth, with width control
     - \ci{Pulse} :: Band-limited pulse wave, with width control
**** Use band-limited waveforms for audio, whenever possible.
     \ci{VarSaw} is not band-limited. It should be OK if \ci{width} is
     close to 0.5 (a triangle wave). \ci{width} = 0 is the same as
     \ci{LFSaw}, and will cause aliasing.

*** Subtractive synthesis: Filter summary
**** Filters so far
     - \ci{LPF} :: Lowpass filter (no resonance)
     - \ci{HPF} :: Highpass filter (no resonance)
     - \ci{RLPF} :: Lowpass filter /with/ resonance
     - \ci{RHPF} :: Highpass filter /with/ resonance
     - \ci{BPF} :: Bandpass filter.
**** Filter quality (Q) and \ci{rq} parameter.
     - Q is a standard measure. SC uses $rq = \frac{1}{Q}$ to save CPU.
     - \ci{RLPF}, \ci{RHPF}, and \ci{BPF} all have a \ci{rq} input.
