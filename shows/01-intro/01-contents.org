...
ERROR: syntax error, unexpected BADTOKEN, expecting ')'
  ...
  rrand(1, 2.0} 
              ^
#+END_EXAMPLE
**** Often, the real mistake comes before the error is actually detected.
...
**** Before beginning, you should install:
     - SuperCollider 3.6.6 or later.\\
       http://supercollider.sourceforge.net/downloads/
     - The /sc3-plugins/ pack: Additional synthesis units.
...
*** XP fail						    :B_ignoreheading:
    :PROPERTIES:
    :BEAMER_env: ignoreheading
    :END:
**** Windows XP only
There's an unfortunate problem with SuperCollider in Windows XP. The
interpreter is not able to launch external processes. This is not an
issue in Windows Vista and later. (Although this is inconvenient for
XP users, since Microsoft no longer supports XP, SuperCollider
developers can't justify the time to fix it.)

In Listing [[ProxyInterfaceCopy]], \ci{openOS} starts an external process
(in Windows, the file explorer). This will not work in XP. Instead,
delete \cd{.openOS} from the line, and run it to get the file
location. Then copy the location out of SC's post window, and paste it
into Windows Explorer. Then proceed with step 2 above.
...
*** XP fail 2						    :B_ignoreheading:
    :PROPERTIES:
    :BEAMER_env: ignoreheading
    :END:
**** Windows XP only
Because of the XP problem, starting the audio server will fail in the
same way. As a workaround:

1. Copy the following code into the startup file.
   #+name: StartupForXP
   #+caption: Add these lines to the startup file if you're using Windows XP.
...
ShutDown.add {
	Server.all.do(_.quit)
};
   #+end_src

2. Create a file =runsynth.bat=, in a location that's easy to find.

3. Copy the following text into it. Be sure to substitute the actual installation path.
   #+name: RunSynthBat
   #+caption: A script to launch the server in Windows XP.
   #+begin_src {}
   cd "C:\Program Files\SuperCollider-3.6.6"
   scsynth.exe -u 57110 -a 128 -b 1026 -R 0
   #+end_src

Now, when you need to boot the server, double-click on this batch
file. The code in Listing [[StartupForXP]] will make the interpreter
recognize when the server has been started externally, and quit the
...
***** A \clss{Function} is a block of code in curly braces.
     - Functions can take information in through \eglspl{argument}.
...
**** How do we get \ci{a} and \ci{b} in there?
     - That's what the \eglspl{argument} are for.
...
***** \te{Declared} variables
     ...
     - \eGlspl{argument} are variables, but get values from outside.
#+name: declarevars
...
***** O2 							  :B_onlyenv:
      ...
      :BEAMER_act: 2
      :END:
****** \eGlspl{class} and \eGlspl{instance}
       - \eGlspl{class} define the structure and behavior.
       - \eGlspl{instance} use the structure to do work.
       - \cd{Point(1, 2)} and \cd{Point(3, 4)} are two \eglspl{instance} of \ci{Point}.\\
       	 In one, \ci{x = 1}; in the other, \ci{x = 3}.\\
       	 \cd{Point(1, 2) + 2} returns one result; \cd{Point(3, 4) + 2}, different.\\
       	 Same class, same operation, different instances, different results.
...
***** O2							  :B_onlyenv:
      ...
      :BEAMER_act: 2
      :END:
****** \eGls{class} and \egls{instance} methods
       - Call /class methods/ directly on a class: \cd{SinOsc.ar}.
       - Often, class methods create instances:
       	 - \cd{SinOsc.ar(440)} makes a \clss{SinOsc} instance.
       	 - \cd{Point.new(1, 2)} makes a \cd{Point} instance.\\
	   \mth{new} is implied: \cd{Point(1, 2)} is the same!
       - After you have the instance, use instance methods on it.\\
       	 \mth{dup} is an instance method, called on the specific \ugen{SinOsc}.
...
**** {1\textwidth}{3in}					      :B_overlayarea:
     ...
     #+end_center
***** \eGlspl{argument} pass information into a method or function.
...
*** Comments
**** /Comments/ are free text in the program body.
     - They are to remind /you/ what you were thinking.
     - The language interpreter ignores them.
     - The IDE highlights comments in red.
     - Easy way: Select text, then \ks{Ctrl-/}.
**** Two ways to write comments:
#+begin_src {SuperCollider} -i
some.code;  // comment: double-/ to end of line

/* Comments may also begin with slash-star,
and end with star-slash.

These comments can span several lines. */

x = 1 + /* or sit in the middle of a line */ 2;
#+end_src
...
**** A \gls{method call} consists of three parts:
    ...
    - \Glspl{argument} :: Additional input (optional).
#+latex: \bigskip
In \cd{SinOsc.ar(440)}:
...
**** Important SC language concepts
     ...
     - \glspl{argument} :: Inputs to a function or method.
...
*** Arguments in synthesis functions
**** Listing [[mod1]] declares two arguments.
     - freq :: Frequency, with a default 440 (A natural).
     - amp :: Amplitude (volume), with a default 0.1 (-20 dB).
**** Arguments are magical in synthesis functions.
     - They automatically become \glspl{control input}.
     - You can set the value from outside, and it changes the sound inside.
     - Try it: \cd{\textasciitilde oscil.set(\textbackslash freq, 261.6)}.
**** Control inputs make it possible to do musical sequencing.
     - The function defines the /structure/ of synthesis.
     - The controls produce specific pitches, volumes etc.
...
**** Add a filter, for a brightness control.
     ...
     - \mth{addSpec} sets the correct range (frequency) for \ci{ffreq}.\\
       More on ranges tomorrow.
#+latex: \glsadd{LPF}
#+name: mod2
...
~out = { ~filter.ar * ~eg }; ~out.play;

~out.stop;
#+End_src

*** Note on 'dup'					    :B_ignoreheading:
    :PROPERTIES:
    :BEAMER_env: ignoreheading
    :END:
**** Why does Listing [[ampenv1]] leave out the \ci{dup} call, which we saw in so many other examples?
Look back to Listing [[mod1]]. \ci{dup} is here. This means that
\ci{\textasciitilde oscil} is already stereo. Tomorrow, we will learn
about a rule called "multichannel expansion." This is a general rule,
which takes some time to appreciate fully. In this specific case, it
means: If you feed a stereo oscillator into a filter, then the filter
will also be stereo---and, if you then multiply the filter by an
envelope, this will be stereo as well.

Since \cd{\textasciitilde filter.ar * \textasciitilde eg} is already
stereo, there is no need to use \ci{dup} to make it stereo again.

When you use any kind of programming language, you create
"identifiers" to represent information or actions. @@latex:\ci{\textasciitilde
oscil}@@ stands for a signal processor that makes a waveform. One of
\ci{\textasciitilde oscil}'s properties is that it's producing a
stereo pair of signals. It's up to you to remember that
\ci{\textasciitilde oscil} is stereo. Then, you can write other signal
processors that depend on \ci{\textasciitilde oscil}'s stereo signal,
so that they will handle the stereo signal properly.

One of the most important skills to use SuperCollider successfully is
to keep the meaning of each identifier clear in your mind. This is a
...
*** Summary: Envelopes
**** \Glspl{envelope} consist of a start value and \eglspl{segment}.
     - Segment: Target level, duration, curve.
     - Written \cd{Env(levels, times, curve)}.\\
       May optionally include a \egls{releaseNode} and \egls{loopNode}.
     - Levels array: \cd{[start, level$_0$, level$_1$, level$_2$...]}.
     - Times array: \cd{[time$_0$, time$_1$, time$_2$...]}.
     - Curve array: \cd{[curve$_0$, curve$_1$, curve$_2$...]}.\\<all>
       \ci{curve} may be a single value, for all segments.
**** Amplitude envelope applies to a signal by multiplying \ci{*}.
     - Usually last in the chain: \cd{finalSignal * ampEg}.

*** Summary: Envelope types
**** \clss{EnvGen} makes a signal from an envelope definition.
     Its \ci{gate} input is especially important. It controls the envelope's start and release.
**** Two main types of envelopes:
     - Timed :: No \gls{releaseNode}.
       - When triggered, it runs according to its own timing.
       - Only needs a momentary \egls{trigger}.
     - Sustaining :: With releaseNode.
       - Holds at the releaseNode as long as the gate is open.
       - A \egls{gate} is open when > 0, closed when $\le$ 0.
...
**** \clsspl{Stream}---\clsspl{Routine}---handle these.
     - Streams have memory.
...
[fn:69c2f54f] "A Short History of Computer Music." Author
