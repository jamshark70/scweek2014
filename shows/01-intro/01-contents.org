...
**** The \clss{Pseries} is more compact, convenient.
...
**** In Listing [[pbind1]], change \clss{Pbind} to \clss{DebugPbind}.
     ...
     2. \ci{\textbackslash freq} gets the value 133.26.
     3. The empty event becomes \cd{(freq: 133.26)}.
...
*** Pattern expresses the same idea
**** They express the same, but in a different style:
     - The \clss{Routine} says:
       - Set the trigger and the frequency.
       - Wait 0.5 beats.
     - The \clss{Pbind} says:
       - Each \clss{Event} has values for trigger, frequency and duration.
       - It's the event's responsibility to handle the synthesis values.
       - It's the \clss{EventStreamPlayer}'s responsibility to play
         the event and handle timing.
