...
**** Same pattern as before:
     - Add a new module (\ci{\textasciitilde filtlfo}).
     - Use it in the filter (\cd{ffreq * \textasciitilde filtlfo}).
#+name: scifi2
...
~filtlfo.addSpec(\lfreq, [0.5, 18, \exp]);
~filtlfo.addSpec(\depth, [0, 4]);
#+end_src
...
**** Remember the key questions from before:
     - What is the range I /get from/ a UGen?
       - \ugen{SinOsc} range: -1--1 (bipolar).
     - What is the range I need to /put into/ a UGen?
       - Frequency is an \gls{exponential} scale.
       - Modulation should be by /multiplying/.
       - 
     - How can I convert one to the other, mathematically?

# later
# #+name: scifi3
# #+caption: Exponential-scale LFO for frequency.
# #+begin_src {} -i
# ~filtlfo = { |lfreq = 1, depth = 0.1|
#    depth = depth + 1;
...
*** Take it apart: How does it work?
**** Here, a simple sinewave.
      \egls{LFO} frequency control ranges from 0.5--18 Hz.
